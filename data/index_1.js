...
function initializeChart(){
  ...
  myChart = new Chart(ctx, {
    type: "line",
    data: {
      datasets: [{ label: "Показать/Скрыть" }],
    },
    options: {
      borderWidth: 3,
      borderColor: ["rgba(255, 99, 132, 1)"], //200, 99, 255
	  xAxes: [{
                ticks: {
                    autoSkip: false,
                    maxRotation: 90,
...
function onMessage(event) {
  ...
  if (/~1~/i.test(event.data)) {
    DatTime = event.data.split("~1~");
    addData(DatTime[0], DatTime[1]);
  //addData(event.data, event.data);
  }
}
